...
sub outputSpanningSeq {
	...
	foreach my $species (keys %species2Chrom) {
		...
		my $seq = getSeqTwoBitFile($species2Chrom{$species}, $startPlus, $endPlus, $species2Strand{$species}, "$twoBitPath/$species$TwoBitSuffix", $unmaskFlag);
		print "retrieve ", $endPlus-$startPlus, " bp sequence for $species $species2Chrom{$species}:$startPlus-$endPlus from 2bit file: $twoBitPath/$species$TwoBitSuffix\n";
		if ($filename ne "") {
...
sub runPrank {
	...
	my $call = "set -o pipefail; perl ConvertTreeModToNewick.perl $treeFile ${tmpFile}.tree.nh";
...
sub addCompleteDeletions {
	...
	my $call = "set -o pipefail; perl ConvertFastaToFastaOneLine.perl ${tmpFile}.best.fas | grep \">\" -v";
	print "$call\n" if ($verbose);
	my @ali = `$call`;
	cleanDie("ERROR: $call failed\n", $tmpFile) if ($? != 0 || ${^CHILD_ERROR_NATIVE} != 0);
